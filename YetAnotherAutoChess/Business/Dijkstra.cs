...
namespace YetAnotherAutoChess.Business
{
    public static class Dijkstra
    {
            ...
            Enums.TargetingSystem targetingSystem = Enums.TargetingSystem.ClosestEnemy)
        {
            ...
            switch (targetingSystem)
            {
                case Enums.TargetingSystem.ClosestEnemy:
                    if (enemies.Count == 0)
                        return null;
                    enemies.Sort((o, e) => Math.Abs(o.Position.Row - e.Position.Row) +
                        Math.Abs(o.Position.Column - e.Position.Column));
                    target = enemies[0];
                    break;
        ...
        public static Point FindNextStep(Figure source, bool isKnight = false)
        {
            ...
            while (EnemyInsideRangeOptimised(source, source.Range, (int)u.X, (int)u.Y) == null)
            {
            ...
            for (int distance = 1; distance <= 8; ++distance)
            {
                for (int i = -distance; i <= distance; i++)
                {
                    ...
                    if (freeTilesInRange.Count > 0)
                        return freeTilesInRange[(new Random()).Next(0, freeTilesInRange.Count)];
                }
            }
